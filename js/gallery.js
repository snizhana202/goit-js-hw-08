...
const onGalleryContainerClick = (event) => {
  ...
  if (
    targetEl.nodeName !== "IMG" ||
    !targetEl.classList.contains("gallery-image")
  ) {
    return;
  }
  ...
  const largeImageUrl = targetEl.dataset.source;
  console.log(largeImageUrl);

  const image = images.find((img) => img.preview === targetEl.src);
  if (!image) return;
  ...
  instance = basicLightbox.create(
    ...
    {
      onShow: (inst) => {
        inst.element().addEventListener("click", () => inst.close());
